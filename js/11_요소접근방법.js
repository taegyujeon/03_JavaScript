...
function classTest2(){
  ...
  /* (중요)
  요소들을 한 번에 얻어오게 되면 배열 형대로 반환 된다!
...
function tagnameTest(){
  ...
  //HTML 문서에 존재하는 모든 li 태 요소를 얻어와 배열로 묶어서 반환
...
function nameTest(){
  ...
  for(let i=0; i<hobbyList.length; i++){ // 순차접근
    ...
    // 체크 여부를 검사해서 체크가 되어 있다면 체크된 요소의 값을 
    // str 변수에 누적 + count 변수 값을 1증가
